 ...
 */
public final class LegacyColorTextParser {
    private LegacyColorTextParser() {
    }

    public static ProcessedText process(String rawText, boolean decodeTinkersPua,
    ...
    public static ProcessedText process(String rawText, boolean decodeTinkersPua,
        ...
        if (rawText == null || rawText.isEmpty()) {
            return ProcessedText.unchanged(rawText);
        }
        ...
        for (int rawIndex = 0; rawIndex < rawText.length();) {
            ...
            if (current == '\u00A7' && rawIndex < rawText.length()) {
                ...
                if (isLegacyColorOrReset(normalizedCode) && colorOverride) {
                    // A vanilla color code replaces Tinkers' GL color just like reset does.
                    // In particular, formatPartialAmount uses section-sign gray for its slash.
                    run.setLength(run.length() - 2);
                    appendRun(modern, run, true, rgb);
                    run.append('\u00A7').append(code);
                    colorOverride = false;
                }
                if (isLegacyColorOrReset(normalizedCode)) {
                    hexGradient = null;
                }
        ...
        if (!transformed) return ProcessedText.unchanged(rawText);
        return ProcessedText.transformed(rawText, visible.toString(), modern.build(),
