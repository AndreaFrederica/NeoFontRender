...
public abstract class AdvancedTabCompleter {
	...
	public AdvancedTabCompleter(GuiTextField textField) {
	...
	public void render(int mouseX, int mouseY, FontRenderer font) {
		...
		if(limit > 0) {
			...
			int x = Math.min(s.length() <= 0 ? 0 : font.getStringWidth(s.substring(0, Math.max(0, textField.getText().lastIndexOf(" ")) + 1)), textField.width) + textField.xPosition;
			int width = 0;
	...
	public void requestUpdate() {
        ...
        ClientCommandHandler.instance.autoComplete(prefix, prefix);
        Minecraft mc = Minecraft.getMinecraft();
        if (mc.thePlayer != null && mc.thePlayer.sendQueue != null) {
            mc.thePlayer.sendQueue.addToSendQueue(new C14PacketTabComplete(prefix));
        }
        cycle = false;
	}
	...
	public void setCompletions(String... newCompl) {
		...
		if (clientCompletions != null && clientCompletions.length > 0) {
			String[] combined = new String[clientCompletions.length + newCompl.length];
			System.arraycopy(clientCompletions, 0, combined, 0, clientCompletions.length);
			System.arraycopy(newCompl, 0, combined, clientCompletions.length, newCompl.length);
			newCompl = combined;
		}
