...
public class GuiSelectColor extends GuiPanel {
    ...
    public GuiSelectColor(final Consumer<Color> callback_, Color color) {
        ...
        label = new GuiLabel();
        label.setText(new TextBuilder().quickTranslate("colors.red").format(EnumChatFormatting.RED).build());
        label.setAngle(300);
        this.addComponent(label, new int[] { 1, 12 });

        label = new GuiLabel();
        label.setText(new TextBuilder().quickTranslate("colors.green").format(EnumChatFormatting.GREEN).build());
        label.setAngle(300);
        this.addComponent(label, new int[] { 4, 12 });

        label = new GuiLabel();
        label.setText(new TextBuilder().quickTranslate("colors.blue").format(EnumChatFormatting.BLUE).build());
        label.setAngle(300);
        this.addComponent(label, new int[] { 7, 12 });

        label = new GuiLabel();
        label.setText(new TextBuilder().quickTranslate("colors.alpha").format(EnumChatFormatting.WHITE).build());
        label.setAngle(300);
