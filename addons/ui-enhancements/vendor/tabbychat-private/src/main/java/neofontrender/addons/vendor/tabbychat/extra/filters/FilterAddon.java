...
public class FilterAddon {
    ...
    public FilterAddon() {
        ...
        setVariable("player", () -> Pattern.quote(mc.thePlayer.getCommandSenderName()));
        setVariable("onlineplayer", () -> Joiner.on('|')
                .appendTo(new StringBuilder("(?:"), mc.theWorld.playerEntities.stream()
                        .map(player -> Pattern.quote(((EntityPlayer) player).getCommandSenderName()))
                        .iterator())
                .append(')').toString()
        );
    }
