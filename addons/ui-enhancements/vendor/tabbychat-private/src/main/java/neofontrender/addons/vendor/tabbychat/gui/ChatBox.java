...
import neofontrender.addons.vendor.tabbychat.api.gui.ChatGui;
import neofontrender.addons.vendor.tabbychat.settings.TabbySettings;
import neofontrender.addons.vendor.tabbychat.util.ScaledDimension;
...
public class ChatBox extends GuiPanel implements ChatGui {
    ...
    public static final ResourceLocation GUI_LOCATION =
            new ResourceLocation("neofontrender_tabbychat", "textures/chatbox.png");
    ...
    public void killjoysMovingCompanyForAllYourFurnitureMovingNeeds(GuiMouseEvent event) {
        ...
        if (event.getType() == MouseEvent.CLICK) {
            ...
            if (Mouse.isButtonDown(0) && (pnlTray.isHovered() || (altDown && isHovered()))) {
    ...
    public void updateComponent() {
        ...
        if (x1 != x || y1 != y || w1 != w || h1 != h) {
            setLocation(new Location(
                    MathHelper.ceiling_float_int(x1 / scale),
                    MathHelper.ceiling_float_int(y1 / scale),
                    MathHelper.ceiling_float_int(w1 / scale),
                    MathHelper.ceiling_float_int(h1 / scale)));
        }
        super.updateComponent();
    }
