...
final class ModernUiTooltipShader {
    ...
    static boolean draw(float left, float top, float right, float bottom, float radius,
        ...
        try {
            ...
            tessellator.addVertex(left - extent, top - extent, 300.0D);
            tessellator.addVertex(left - extent, bottom + extent, 300.0D);
            tessellator.addVertex(right + extent, bottom + extent, 300.0D);
            tessellator.addVertex(right + extent, top - extent, 300.0D);
            tessellator.draw();
            return true;
        } catch (Throwable t) {
            unavailable = true;
