...
public final class HoverEffectsRenderer {
    ...
    public static float visualProgress(GuiButton button) {
        if (!HoverEffectsConfigAccess.buttonsEnabled()) return button.isMouseOver() && button.enabled ? 1.0F : 0.0F;
    ...
    public static void drawIngredientGridHighlight(Rectangle area, float progress) {
        ...
        GuiUtils.drawGradientRect(0, area.x, area.y, area.x + area.width, area.y + area.height,
                color, color);
    ...
    public static void applyForgeButtonAlpha() {
        if (forgeButtonAlpha < 0.999F) {
            GlStateManager.color(1.0F, 1.0F, 1.0F, forgeButtonAlpha);
        }
    }
