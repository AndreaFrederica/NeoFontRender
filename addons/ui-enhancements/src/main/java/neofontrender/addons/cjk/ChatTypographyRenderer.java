...
public final class ChatTypographyRenderer {
    ...
    public static boolean isPositioned(ITextComponent component) {
        return component instanceof PositionedTextLine;
    }

    public static int draw(FontRenderer font, ITextComponent component, float x, float y,
                           int color, boolean shadow) {
        if (!(component instanceof PositionedTextLine)) {
            String text = component == null ? "" : component.getFormattedText();
            return shadow ? font.drawStringWithShadow(text, x, y, color)
                    : font.drawString(text, x, y, color, false);
        }
        PositionedTextLine line = (PositionedTextLine) component;
        for (TextParagraphProvider.Run run : line.nfrUi$runs()) {
    ...
    public static int width(FontRenderer font, ITextComponent component) {
        return component instanceof PositionedTextLine
                ? (int) Math.ceil(((PositionedTextLine) component).nfrUi$width())
                : font.getStringWidth(component == null ? "" : component.getFormattedText());
    }

    public static ITextComponent componentAt(ITextComponent component, float x) {
        return component instanceof PositionedTextLine
                ? ((PositionedTextLine) component).nfrUi$componentAt(x) : null;
    }

    public static int formattedIndexAt(ITextComponent component, float x) {
    ...
    public static float xAtFormattedIndex(ITextComponent component, int formattedIndex) {
