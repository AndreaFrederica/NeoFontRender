...
public final class CjkTypographyRenderer {
    ...
    public static TextParagraphProvider.Layout layout(
    ...
    public static List<ITextComponent> splitComponents(
            ...
            TextParagraphProvider.ComponentRequest.Surface surface) {
        if (!NeofontrenderConfig.fixCjkLineBreak() || component == null) return null;
        return TextParagraphApi.splitComponents(
    ...
    public static int measuredWidth(FontRenderer font, String text) {
        TextParagraphProvider.Layout layout = layout(
                font, text, UNBOUNDED_GUI_WIDTH, font.FONT_HEIGHT);
        return layout == null ? font.getStringWidth(text) : measuredWidth(font, layout);
    }

    public static int measuredWidth(FontRenderer font, TextParagraphProvider.Layout layout) {
        float right = 0;
    ...
    public static List<String> wrap(FontRenderer font, String text, int width, int lineHeight) {
        TextParagraphProvider.Layout layout = layout(font, text, width, lineHeight);
    ...
    public static boolean draw(FontRenderer font, TextParagraphProvider.Layout layout,
    ...
    public static boolean draw(FontRenderer font, TextParagraphProvider.Layout layout,
        ...
        for (int lineIndex = 0; lineIndex < count; lineIndex++) {
            TextParagraphProvider.Line line = layout.lines().get(lineIndex);
            for (TextParagraphProvider.Run run : line.runs()) {
