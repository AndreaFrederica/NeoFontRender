 ...
 * loading screen does not create a competing GPU device while the game is still starting.</p>
 */
final class Arc3DLoadingBarRenderer {
    ...
    private static void quad(float left, float top, float right, float bottom,
        ...
        vertex(tessellator, left, top, leftColor);
        vertex(tessellator, left, bottom, leftColor);
        vertex(tessellator, right, bottom, rightColor);
        vertex(tessellator, right, top, rightColor);
        tessellator.draw();
    }
