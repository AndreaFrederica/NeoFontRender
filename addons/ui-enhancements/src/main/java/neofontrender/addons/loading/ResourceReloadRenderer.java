...
public enum ResourceReloadRenderer {
    ...
    private void requestFrame(boolean force) {
        ...
        if (force) mc.loadingScreen.displayLoadingString("");
    ...
    private static void drawVerticalGradient(int left, int top, int right, int bottom,
        ...
        try {
            ...
            for (int segment = 0; segment < MATERIAL_GRADIENT_SEGMENTS; segment++) {
                ...
                buffer.pos(right, firstY, 0.0D)
                        .color(lerp(startR, endR, firstAmount), lerp(startG, endG, firstAmount),
                                lerp(startB, endB, firstAmount), lerp(startA, endA, firstAmount)).endVertex();
                buffer.pos(left, firstY, 0.0D)
                        .color(lerp(startR, endR, firstAmount), lerp(startG, endG, firstAmount),
                                lerp(startB, endB, firstAmount), lerp(startA, endA, firstAmount)).endVertex();
                buffer.pos(left, secondY, 0.0D)
                        .color(lerp(startR, endR, secondAmount), lerp(startG, endG, secondAmount),
                                lerp(startB, endB, secondAmount), lerp(startA, endA, secondAmount)).endVertex();
                buffer.pos(right, secondY, 0.0D)
                        .color(lerp(startR, endR, secondAmount), lerp(startG, endG, secondAmount),
                                lerp(startB, endB, secondAmount), lerp(startA, endA, secondAmount)).endVertex();
            }
