...
/**
 * Independent 1.7.10 implementation of a bottom-anchored world-loading presentation.
 *
...
public enum WorldLoadingRenderer {
    ...
    public void serverWorldLoaded(WorldEvent.Load event) {
        if (!(event.world instanceof WorldServer) || event.world.provider.dimensionId != 0) return;
        if (!(MinecraftServer.getServer() instanceof IntegratedServer)) return;
        IntegratedServer server = (IntegratedServer) MinecraftServer.getServer();
    ...
    public void beforeScreenDraw(GuiScreenEvent.DrawScreenEvent.Pre event) {
        if (event.gui instanceof GuiScreenWorking
            ...
            render(event.gui.width, event.gui.height, 0.99F, 1.0F, System.nanoTime());
            return;
        }
        if (!(event.gui instanceof GuiDownloadTerrain) || !WorldLoadingConfig.enabled) return;
        ...
        lastDrawNanos = now;
        render(event.gui.width, event.gui.height, displayedProgress, 1.0F, now);
    }

    @SubscribeEvent(priority = EventPriority.LOWEST)
    public void afterGameOverlay(RenderGameOverlayEvent.Post event) {
        if (event.type != RenderGameOverlayEvent.ElementType.ALL || !fading) return;
        Minecraft mc = Minecraft.getMinecraft();
        if (mc.theWorld == null || mc.currentScreen instanceof GuiDownloadTerrain) return;
    ...
    public void clientTick(TickEvent.ClientTickEvent event) {
        ...
        if (mc.theWorld == null || mc.thePlayer == null || mc.currentScreen instanceof GuiDownloadTerrain) return;
        integratedLaunchActive = false;
        stableDimension = mc.thePlayer.dimension;
        hasStableWorld = true;
    }
    ...
    public void beginClientWorldLoad(WorldClient nextWorld) {
        ...
        Minecraft mc = Minecraft.getMinecraft();
        boolean dimensionChange = hasStableWorld && mc.thePlayer != null
                && nextWorld.provider.dimensionId != stableDimension;
    ...
    private void requestRendererPreparationFrame(boolean force) {
        ...
        Minecraft mc = Minecraft.getMinecraft();
        if (mc.loadingScreen == null || !mc.func_152345_ab()) return;
     ...
     * the initial world. Vanilla prepares exactly 625 spawn chunks in 1.7.10; chunk events provide
    ...
    private void begin(Minecraft mc, long now) {
        boolean dimensionChange = hasStableWorld && mc.thePlayer != null
                && mc.thePlayer.dimension != stableDimension;
    ...
    private static int loadedChunkCount() {
        Minecraft mc = Minecraft.getMinecraft();
        if (mc.theWorld == null || mc.theWorld.getChunkProvider() == null) return 0;
        if (!(mc.theWorld.getChunkProvider() instanceof ChunkProviderClient)) return 0;
        return Math.max(0, ((AccessorChunkProviderClient) mc.theWorld.getChunkProvider())
                .nfrUi$getLoadedChunks().getNumHashElements());
    }
    ...
    private static void drawVanillaBackground(int width, int height, float opacity) {
        ...
        Tessellator tessellator = Tessellator.instance;
        tessellator.startDrawingQuads();
        tessellator.setColorRGBA(64, 64, 64, alpha);
        tessellator.addVertexWithUV(0.0D, height, 0.0D, 0.0D, height / 32.0F);
        tessellator.addVertexWithUV(width, height, 0.0D, width / 32.0F, height / 32.0F);
        tessellator.addVertexWithUV(width, 0.0D, 0.0D, width / 32.0F, 0.0D);
