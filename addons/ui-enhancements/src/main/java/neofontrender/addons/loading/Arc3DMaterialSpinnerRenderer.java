...
final class Arc3DMaterialSpinnerRenderer {
    ...
    private static void drawArc(float centerX, float centerY, float startDegrees,
        ...
        for (int i = 1; i <= segments; i++) {
            ...
            arcQuad(tessellator, centerX, centerY, previous, next, color);
            previous = next;
        }
        ...
        roundCap(tessellator, centerX, centerY, start, middleRadius, capRadius, false, color);
        roundCap(tessellator, centerX, centerY, end, middleRadius, capRadius, true, color);
        tessellator.draw();
    }

    private static void arcQuad(Tessellator tessellator, float cx, float cy,
        ...
        vertex(tessellator, foX, foY, color); vertex(tessellator, fiX, fiY, color);
        vertex(tessellator, soX, soY, color);
        vertex(tessellator, soX, soY, color); vertex(tessellator, fiX, fiY, color);
        vertex(tessellator, siX, siY, color);
    }

    private static void roundCap(Tessellator tessellator, float cx, float cy, float angle,
        ...
        for (int i = 0; i < capSegments; i++) {
            ...
            vertex(tessellator, px, py, color);
            capVertex(tessellator, px, py, nx, ny, tx, ty, radius, a, end, color);
            capVertex(tessellator, px, py, nx, ny, tx, ty, radius, b, end, color);
        }
    }

    private static void capVertex(Tessellator tessellator, float px, float py,
        ...
        vertex(tessellator, px + radius * (normal * nx + tangent * tx),
