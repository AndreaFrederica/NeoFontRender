...
public final class NfrTabbySettingsBridge {
    ...
    public static void loadGlobal(TabbySettings settings) {
        ...
        g.timestampColor.set(enumValue(EnumChatFormatting.class, f.getString("chat.tabby.general.timestampColor", EnumChatFormatting.WHITE.name()), EnumChatFormatting.WHITE));
    ...
    public static void loadServer(ServerSettings settings, String serverKey) {
        ...
        g.ignoredChannels.set(f.getStringList(p + "ignoredChannels", Collections.emptyList()));
    ...
    public static void saveServer(ServerSettings settings, String serverKey) {
        ...
        UiEnhancementsConfig.file()
                ...
                .set(p + "ignoredChannels", new ArrayList<>(g.ignoredChannels.get()))
    ...
    private static void defineGlobal(NfrConfigFile f) {
        f.define("chat.tabby.general.logChat", true, "Write chat logs.")
                ...
                .define("chat.tabby.general.timestampColor", EnumChatFormatting.WHITE.name(), "Timestamp color.")
    ...
    private static <E extends Enum<E>> E enumValue(Class<E> type, String value, E fallback) {
        try {
            ...
            NfrUiEnhancements.LOGGER.warn("Invalid enum value '{}' for {}; using {}", value, type.getSimpleName(), fallback, exception);
            return fallback;
        }
    }

    private static <T> T json(String value, Type type, T fallback) {
        try {
            T parsed = GSON.fromJson(value, type);
            return parsed == null ? fallback : parsed;
        } catch (RuntimeException exception) {
            NfrUiEnhancements.LOGGER.warn("Invalid TabbyChat settings JSON; using default value", exception);
            return fallback;
        }
    }
}
