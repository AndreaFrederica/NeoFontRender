...
public final class FlightRollController implements FlightRollNetwork.ClientListener, FlightApi.Backend {
    ...
    public void mouseInput(CameraMouseInputEvent event) {
        ...
        float keyboardYaw = (YAW_RIGHT.getIsKeyPressed() ? 1.0F : 0.0F)
                - (YAW_LEFT.getIsKeyPressed() ? 1.0F : 0.0F);
    ...
    public static float cameraRoll(float partialTicks) {
        if (INSTANCE.mc.thePlayer == null) return 0.0F;
        FlightCameraTracking tracking = FlightApi.queryCameraTracking(
                INSTANCE.mc.thePlayer, partialTicks);
        if (tracking == null) return INSTANCE.renderedRoll(partialTicks);
        return INSTANCE.trackedCameraRoll;
    }

    @SubscribeEvent
    public void channelRegistration(FMLNetworkEvent.CustomPacketRegistrationEvent<?> event) {
        if (event.side != Side.CLIENT
                || !event.registrations.contains(FlightRollNetwork.CHANNEL_NAME)) return;
        if ("REGISTER".equals(event.operation) && !handshakeRequested) {
    ...
    private void applyLocalRotation(EntityPlayerSP player, double pitchDegrees,
        ...
        player.setAngles(yawDelta / 0.15F, -pitchDelta / 0.15F);
    ...
    private boolean capability(EntityPlayerSP player, FlightCapability capability,
        ...
        if (decision == FlightDecision.ALLOW) event.setResult(Event.Result.ALLOW);
        else if (decision == FlightDecision.DENY) event.setResult(Event.Result.DENY);
    ...
    static float hudPitch() { return INSTANCE.mc.thePlayer == null ? 0.0F : INSTANCE.mc.thePlayer.rotationPitch; }
    ...
    static float playerRollForEntity(int entityId, float partialTicks) {
        if (INSTANCE.mc.thePlayer != null && INSTANCE.mc.thePlayer.getEntityId() == entityId) {
            boolean enabled = INSTANCE.capability(INSTANCE.mc.thePlayer,
                    FlightCapability.PLAYER_ROLL_RENDERING, INSTANCE.active(INSTANCE.mc.thePlayer));
    ...
    @Override public boolean startBarrelRoll(int direction, int durationTicks) {
        if (!active(mc.thePlayer) || barrelProgress < 1.0F) return false;
