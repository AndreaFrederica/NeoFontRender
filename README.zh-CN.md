...
- 可选等宽单字缓存：支持等宽 ASCII、CJK 和等宽数字，按实际排版结果保留连字与字距。缓存设置页可开关（默认关闭）并调整独立上限（默认 16,384 条）；也可设置 `performance.monospaceCharacterCache` 与 `performance.monospaceCharacterCacheMaxEntries`。F3 显示纹理/测宽占用及纹理命中、未命中、淘汰次数，普通构建也会计数。
- 实验性异步字体渲染：缓存设置页的独立开关 `performance.asyncFontRendering` 默认关闭，适用于所有 Cosmic 文本。借鉴 Typst 的后台光栅化方式，用独立字体引擎处理光栅化所需的排版、阴影/SDF 和字符拆分探测；准确测宽、动画簇查询及 GL 上传/绘制仍同步执行。等待时用原版字体临时显示当前文字并保持 Cosmic 宽度，可能短暂变字体。每个渲染器最多保留 128 个后台任务、32 MiB 待上传数据；上传按约 16.7 ms 窗口限制为最多 8 次/约 1 ms（单次上传不可中断）。F3 显示队列、执行中、就绪、完成、丢弃及失败数据；异步模式的 char probe enqueue 统计主线程提交开销。主要改善冷缓存卡顿，不减少绘制调用，也不保证平均 FPS 上升；独立引擎会增加内存占用。
- 单字探测采用持续时间预算，预算不足时暂用词缓存；连续稳定显示的文本会恢复合并纹理。字体特征按字号/样式 LRU 保留，F3 的 `char probe` 显示探测次数与耗时（每秒）、暂缓次数和稳定文本合并次数。
